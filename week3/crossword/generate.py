...
class CrosswordCreator():
        ...
        for variable, word in assignment.items():
            ...
            for variable2, word2 in assignment.items():
                if variable != variable2:
                    # Check if there are no duplicate words in the solution
                    if word == word2:
                        return False
                    # Get the overlap of the 2 variables
                    overlap = self.crossword.overlaps[variable, variable2]
                    if overlap:
                        x, y = overlap
                        # Check if the overlapping letters are the same
                        if word[x] != word2[y]:
                            return False
        return True
    ...
    def order_domain_values(self, var, assignment):
        ...
        for val in self.domains[var]:
            values[val] = 0
            # Get all neighbours that are not already in the assignment
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables - set(assignment):
            if (
            best_var is None 
            or len(self.domains[var]) < len(self.domains[best_var]) 
            or len(self.crossword.neighbors(var)) > len(self.crossword.neighbors(best_var))
            ):
                best_var = var
        return best_var
